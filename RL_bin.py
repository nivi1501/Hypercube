...
def binary_reward_function(current_matrix, original_matrix):
    # Reward of 1 if matrices are the same, else reward of 0 (or a negative penalty)
    if torch.equal(current_matrix, original_matrix):
        return 1  # Positive reward for exact match
    else:
        return -1  # Negative reward or penalty for incorrect match
...
def q_learning_binary(matrix, original_matrix, dim, max_iterations=10000):
